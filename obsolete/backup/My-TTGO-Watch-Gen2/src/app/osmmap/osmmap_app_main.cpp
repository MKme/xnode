...
void osmmap_app_main_setup( uint32_t tile_num ) {
    ...
    osmmap_ext_pos_img = lv_img_create( osmmap_cont, NULL );
    lv_img_set_src( osmmap_ext_pos_img, &info_ok_16px );
    lv_obj_align( osmmap_ext_pos_img, osmmap_cont, LV_ALIGN_IN_TOP_LEFT, 120, 120 );
    ...
    if ( eventmask & OSM_APP_UPDATE_REQUEST ) {
        ...
        if ( osmmap_external_marker_valid ) {
            ...
            if ( osm_map_project_lon_lat( osmmap_location, osmmap_external_marker_lon, osmmap_external_marker_lat, &marker_x, &marker_y ) ) {
                lv_obj_align( osmmap_ext_pos_img, lv_obj_get_parent( osmmap_ext_pos_img ), LV_ALIGN_IN_TOP_LEFT, marker_x - 8 , marker_y - 8 );
                lv_obj_set_hidden( osmmap_ext_pos_img, false );
            }
            else {
                lv_obj_set_hidden( osmmap_ext_pos_img, true );
            }
