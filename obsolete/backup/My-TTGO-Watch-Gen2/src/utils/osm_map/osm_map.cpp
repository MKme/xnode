...
bool osm_map_update( osm_location_t *osm_location ) {
    ...
    if ( osm_location->tiley == osm_helper_lat2tiley( osm_location->lat, osm_location->zoom ) && osm_location->tilex == osm_helper_long2tilex( osm_location->lon, osm_location->zoom ) ) {
        ...
        osm_location->tiley_pos = fabs( osm_location->tiley_left_top_edge - osm_location->lat ) / osm_location->tiley_px_res;
        osm_location->tilex_pos = fabs( osm_location->tilex_left_top_edge - osm_location->lon ) / osm_location->tilex_px_res;
...
bool osm_map_project_lon_lat( osm_location_t *osm_location, double lon, double lat, uint16_t *x, uint16_t *y ) {
    ...
    osm_map_take( osm_location );
    if ( osm_location->tilex_px_res > 0.0 && osm_location->tiley_px_res > 0.0 &&
        ...
        *x = (uint16_t)( fabs( osm_location->tilex_left_top_edge - lon ) / osm_location->tilex_px_res );
        *y = (uint16_t)( fabs( osm_location->tiley_left_top_edge - lat ) / osm_location->tiley_px_res );
        in_view = true;
    }
    osm_map_give( osm_location );
