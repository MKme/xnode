...
void meshtastic_app_setup( void ) {
    ...
    meshtastic_app = app_register( "mesh\nchat", &message_64px, enter_meshtastic_app_event_cb );

    lv_obj_t *header = wf_add_settings_header( meshtastic_app_tile, "meshtastic" );
    ...
    lv_label_set_long_mode( meshtastic_status_label, LV_LABEL_LONG_BREAK );
    ...
    lv_label_set_long_mode( meshtastic_last_label, LV_LABEL_LONG_BREAK );
    ...
    lv_obj_align( meshtastic_input, meshtastic_last_label, LV_ALIGN_OUT_BOTTOM_LEFT, 0, THEME_PADDING * 2 );
    lv_obj_set_event_cb( meshtastic_input, meshtastic_input_event_cb );

    lv_obj_t *send_btn = lv_btn_create( meshtastic_app_tile, NULL );
    lv_obj_add_style( send_btn, LV_BTN_PART_MAIN, ws_get_button_style() );
    lv_obj_set_size( send_btn, ( lv_disp_get_hor_res( NULL ) / 2 ) - ( THEME_PADDING * 2 ), 48 );
    lv_obj_align( send_btn, meshtastic_input, LV_ALIGN_OUT_BOTTOM_LEFT, 0, THEME_PADDING );
    lv_obj_set_event_cb( send_btn, meshtastic_send_event_cb );
    lv_obj_t *send_label = lv_label_create( send_btn, NULL );
    lv_label_set_text( send_label, "send" );
    lv_obj_align( send_label, send_btn, LV_ALIGN_CENTER, 0, 0 );

    lv_obj_t *inbox_btn = lv_btn_create( meshtastic_app_tile, NULL );
    lv_obj_add_style( inbox_btn, LV_BTN_PART_MAIN, ws_get_button_style() );
    lv_obj_set_size( inbox_btn, ( lv_disp_get_hor_res( NULL ) / 2 ) - ( THEME_PADDING * 2 ), 48 );
    lv_obj_align( inbox_btn, send_btn, LV_ALIGN_OUT_RIGHT_MID, THEME_PADDING, 0 );
    lv_obj_set_event_cb( inbox_btn, meshtastic_inbox_event_cb );
    lv_obj_t *inbox_label = lv_label_create( inbox_btn, NULL );
    lv_label_set_text( inbox_label, "inbox" );
    lv_obj_align( inbox_label, inbox_btn, LV_ALIGN_CENTER, 0, 0 );

    lv_obj_t *exit_btn = lv_btn_create( meshtastic_app_tile, NULL );
    lv_obj_add_style( exit_btn, LV_BTN_PART_MAIN, ws_get_button_style() );
    lv_obj_set_size( exit_btn, lv_disp_get_hor_res( NULL ) - ( THEME_PADDING * 2 ), 44 );
    lv_obj_align( exit_btn, send_btn, LV_ALIGN_OUT_BOTTOM_LEFT, 0, THEME_PADDING );
    lv_obj_set_event_cb( exit_btn, exit_meshtastic_app_event_cb );
    lv_obj_t *exit_label = lv_label_create( exit_btn, NULL );
    lv_label_set_text( exit_label, "back" );
    lv_obj_align( exit_label, exit_btn, LV_ALIGN_CENTER, 0, 0 );
...
static void meshtastic_app_refresh( void ) {
    ...
    snprintf( buf, sizeof( buf ), "node !%08" PRIX32, meshtastic_service_get_node_id() );
    ...
    if ( meshtastic_service_get_last_peer() != 0 ) {
        ...
        snprintf( buf, sizeof( buf ), "public LongFast  US 906.875MHz" );
    }
