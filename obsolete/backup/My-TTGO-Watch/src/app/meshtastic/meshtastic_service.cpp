...
#include "meshtastic_service.h"
    ...
    namespace {
        ...
        bool meshtastic_decode_text_message( const uint8_t *buffer, size_t len, meshtastic_decoded_text_t &decoded ) {
            size_t offset = 0;
            ...
            while ( offset < len ) {
                ...
                switch( field_num ) {
                    ...
                    case 2: {
                        ...
                        if ( wire_type != 2 || !meshtastic_read_varint( buffer, len, offset, text_len ) ) {
                            return( false );
                        }
                        if ( offset + text_len > len ) {
                            return( false );
                        }
                        decoded.text_len = text_len < ( sizeof( decoded.text ) - 1 ) ? text_len : ( sizeof( decoded.text ) - 1 );
                        memcpy( decoded.text, &buffer[ offset ], decoded.text_len );
            ...
            decoded.valid = decoded.portnum == MESHTASTIC_TEXT_MESSAGE_APP && decoded.text_len > 0;
        ...
        bool meshtastic_handle_rx( void ) {
            ...
            if ( !meshtastic_decode_text_message( payload, payload_len, decoded ) ) {
                return( false );
            }

            if ( decoded.dest != 0 && decoded.dest != MESHTASTIC_BROADCAST && decoded.dest != meshtastic_node_id ) {
                return( false );
            }
            ...
            meshtastic_update_status( "RX %s", sender );
            return( true );
        }
