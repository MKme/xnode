    ...
    void IRController_setup( void ) {
...
#else
    #if defined( M5PAPER ) ||  defined( M5CORE2 ) || defined( LILYGO_WATCH_ULTRA ) || defined( LILYGO_T_DECK_PLUS ) || defined( LILYGO_T_DECK_PRO ) || defined( LILYGO_WATCH_2021 ) || defined( WT32_SC01 )
        void IRController_setup( void ) {
            return;
        }
            ...
            #include <LilyGoLib.h>
            #define WATCH_IR_PIN BOARD_IR_PIN
        #else
            #include <TTGO.h>
            #define WATCH_IR_PIN TWATCH_2020_IR_PIN
        #endif
        ...
        */
        ...
        void execute_ir_cmd(InfraButton* config) {
            pinMode(WATCH_IR_PIN, OUTPUT);
            digitalWrite(WATCH_IR_PIN, LOW); // No Current Limiting so keep it off (!!!)
