#include "config.h"
#include "xnode_sos.h"

#include "gui/app.h"
#include "gui/mainbar/mainbar.h"
#include "gui/widget_factory.h"
#include "gui/widget_styles.h"
#include "hardware/ble/xnode.h"
#include "hardware/button.h"

#ifdef NATIVE_64BIT
    #include "utils/logging.h"
    #include "utils/millis.h"
#else
    #include <Arduino.h>
#endif

LV_IMG_DECLARE( notification_64px );

namespace {
    icon_t *xnode_sos_app = NULL;
    lv_obj_t *xnode_sos_tile = NULL;
    lv_obj_t *xnode_sos_exit_btn = NULL;
    lv_obj_t *xnode_sos_send_btn = NULL;
    lv_obj_t *xnode_sos_status_label = NULL;
    uint32_t xnode_sos_tile_num = 0;

    bool xnode_sos_accept_button_event( lv_event_t event, uint32_t &last_event_ms, uint32_t debounce_ms ) {
        #if defined( LILYGO_WATCH_ULTRA )
            if ( event != LV_EVENT_PRESSED ) {
                return( false );
            }
            const uint32_t now = millis();
            if ( last_event_ms != 0 && now - last_event_ms < debounce_ms ) {
                return( false );
            }
            last_event_ms = now;
            return( true );
        #else
            return( event == LV_EVENT_CLICKED );
        #endif
    }

    void xnode_sos_configure_image_button( lv_obj_t *button ) {
        #if defined( LILYGO_WATCH_ULTRA )
            if ( !button ) {
                return;
            }

            const lv_coord_t target_size = 78;
            if ( lv_obj_get_width( button ) < target_size ) {
                lv_obj_set_width( button, target_size );
            }
            if ( lv_obj_get_height( button ) < target_size ) {
                lv_obj_set_height( button, target_size );
            }

            lv_obj_t *icon = lv_obj_get_child( button, NULL );
            if ( icon ) {
                lv_obj_align( icon, button, LV_ALIGN_CENTER, 0, 0 );
            }
            lv_obj_set_ext_click_area( button, 18, 18, 18, 18 );
            lv_obj_move_foreground( button );
        #endif
    }

    void xnode_sos_configure_action_button( lv_obj_t *button ) {
        #if defined( LILYGO_WATCH_ULTRA )
            if ( !button ) {
                return;
            }

            const lv_coord_t target_height = 72;
            if ( lv_obj_get_height( button ) < target_height ) {
                lv_obj_set_height( button, target_height );
            }
            lv_obj_set_ext_click_area( button, 18, 18, 18, 18 );
        #endif
    }

    void xnode_sos_configure_controls( void ) {
        xnode_sos_configure_action_button( xnode_sos_send_btn );
        xnode_sos_configure_image_button( xnode_sos_exit_btn );
        #if defined( LILYGO_WATCH_ULTRA )
            if ( xnode_sos_exit_btn ) {
                lv_obj_align( xnode_sos_exit_btn, xnode_sos_tile, LV_ALIGN_IN_BOTTOM_LEFT, THEME_PADDING * 3, -( THEME_PADDING * 3 ) );
            }
        #endif
    }

    void xnode_sos_exit_to_previous( void ) {
        mainbar_jump_back();
    }

    void xnode_sos_set_status( const char *text ) {
        if ( xnode_sos_status_label ) {
            lv_label_set_text( xnode_sos_status_label, text ? text : "" );
        }
    }

    void xnode_sos_open_event_cb( lv_obj_t *obj, lv_event_t event ) {
        switch( event ) {
            case LV_EVENT_CLICKED:
                xnode_sos_set_status( "Ready" );
                mainbar_jump_to_tilenumber( xnode_sos_tile_num, LV_ANIM_OFF, true );
                break;
            default:
                break;
        }
    }

    void xnode_sos_exit_event_cb( lv_obj_t *obj, lv_event_t event ) {
        static uint32_t last_event_ms = 0;
        if ( xnode_sos_accept_button_event( event, last_event_ms, 250 ) ) {
            xnode_sos_exit_to_previous();
        }
    }

    void xnode_sos_send_event_cb( lv_obj_t *obj, lv_event_t event ) {
        static uint32_t last_event_ms = 0;
        if ( xnode_sos_accept_button_event( event, last_event_ms, 1000 ) ) {
            xnode_sos_set_status( xnode_send_manual_sos() ? "SOS sent over mesh" : "SOS not sent" );
        }
    }

    bool xnode_sos_button_event_cb( EventBits_t event, void *arg ) {
        switch( event ) {
            case BUTTON_EXIT:
                xnode_sos_exit_to_previous();
                break;
            default:
                break;
        }
        return( true );
    }
}

void xnode_sos_tile_setup( void ) {
    const lv_coord_t content_width = lv_disp_get_hor_res( NULL ) - ( THEME_PADDING * 2 );
    #if defined( LILYGO_WATCH_ULTRA )
        const lv_coord_t send_height = 72;
    #else
        const lv_coord_t send_height = 56;
    #endif

    xnode_sos_tile_num = mainbar_add_app_tile( 1, 1, "Manual SOS" );
    xnode_sos_tile = mainbar_get_tile_obj( xnode_sos_tile_num );
    lv_obj_add_style( xnode_sos_tile, LV_OBJ_PART_MAIN, ws_get_app_opa_style() );

    #if defined( LILYGO_WATCH_ULTRA )
        xnode_sos_exit_btn = wf_add_exit_button( xnode_sos_tile, xnode_sos_exit_event_cb );
        lv_obj_align( xnode_sos_exit_btn, xnode_sos_tile, LV_ALIGN_IN_BOTTOM_LEFT, THEME_PADDING * 3, -( THEME_PADDING * 3 ) );
    #else
        xnode_sos_exit_btn = wf_add_close_button( xnode_sos_tile, xnode_sos_exit_event_cb );
        lv_obj_align( xnode_sos_exit_btn, xnode_sos_tile, LV_ALIGN_IN_TOP_RIGHT, -THEME_PADDING, THEME_PADDING );
    #endif

    lv_obj_t *title = lv_label_create( xnode_sos_tile, NULL );
    lv_obj_add_style( title, LV_OBJ_PART_MAIN, APP_STYLE );
    lv_label_set_text( title, "Manual SOS" );
    lv_obj_align( title, xnode_sos_tile, LV_ALIGN_IN_TOP_LEFT, THEME_PADDING, THEME_PADDING );

    lv_obj_t *summary = lv_label_create( xnode_sos_tile, NULL );
    lv_obj_add_style( summary, LV_OBJ_PART_MAIN, APP_STYLE );
    lv_label_set_long_mode( summary, LV_LABEL_LONG_BREAK );
    lv_obj_set_width( summary, content_width );
    lv_label_set_text( summary, "P1 HELP SITREP\nManual SOS" );
    lv_obj_set_width( summary, content_width );
    lv_obj_align( summary, title, LV_ALIGN_OUT_BOTTOM_LEFT, 0, THEME_PADDING );

    xnode_sos_send_btn = lv_btn_create( xnode_sos_tile, NULL );
    lv_obj_add_style( xnode_sos_send_btn, LV_BTN_PART_MAIN, ws_get_button_style() );
    lv_obj_set_size( xnode_sos_send_btn, content_width, send_height );
    lv_obj_align( xnode_sos_send_btn, summary, LV_ALIGN_OUT_BOTTOM_LEFT, 0, THEME_PADDING * 2 );
    lv_obj_set_event_cb( xnode_sos_send_btn, xnode_sos_send_event_cb );

    lv_obj_t *send_label = lv_label_create( xnode_sos_send_btn, NULL );
    lv_label_set_text( send_label, "SEND SOS" );
    lv_obj_align( send_label, xnode_sos_send_btn, LV_ALIGN_CENTER, 0, 0 );

    xnode_sos_status_label = lv_label_create( xnode_sos_tile, NULL );
    lv_obj_add_style( xnode_sos_status_label, LV_OBJ_PART_MAIN, APP_STYLE );
    lv_label_set_long_mode( xnode_sos_status_label, LV_LABEL_LONG_DOT );
    lv_obj_set_width( xnode_sos_status_label, content_width );
    lv_label_set_text( xnode_sos_status_label, "Ready" );
    lv_obj_set_width( xnode_sos_status_label, content_width );
    lv_obj_align( xnode_sos_status_label, xnode_sos_send_btn, LV_ALIGN_OUT_BOTTOM_LEFT, 0, THEME_PADDING );

    xnode_sos_configure_controls();

    mainbar_add_tile_button_cb( xnode_sos_tile_num, xnode_sos_button_event_cb );
    xnode_sos_app = app_register( "SOS", &notification_64px, xnode_sos_open_event_cb );
}
