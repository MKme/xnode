    ...
    portEXIT_CRITICAL(&PMU_IRQ_Mux);
        ...
        if ( temp_pmu_irq_flag ) {
            ...
            if ( watch.isBatChagerStartIrq() ) {
                log_d("AXP2101: ChargingStartIrq");
                powermgm_set_event( POWERMGM_WAKEUP_REQUEST );
                charging = true;
            }
...
void pmu_shutdown( void ) {
    ...
    #elif defined( M5CORE2 )
        M5.shutdown();
    #elif defined( LILYGO_WATCH_S3 ) || defined( LILYGO_WATCH_ULTRA ) || defined( LILYGO_T_DECK_PLUS ) || defined( LILYGO_T_DECK_PRO )
        watch.shutdown();
    #elif defined( LILYGO_WATCH_2020_V1 ) || defined( LILYGO_WATCH_2020_V2 ) || defined( LILYGO_WATCH_2020_V3 )
        TTGOClass *ttgo = TTGOClass::getWatch();
        ttgo->power->shutdown();
...
void pmu_set_high_charging_target_voltage( bool value ) {
        ...
        if ( pmu_config.high_charging_target_voltage ) {
            watch.setChargeTargetVoltage( XPOWERS_AXP2101_CHG_VOL_4V35 );
        }
        else {
            watch.setChargeTargetVoltage( XPOWERS_AXP2101_CHG_VOL_4V2 );
        }
    #elif defined( LILYGO_T_DECK_PLUS )
        pmu_config.high_charging_target_voltage = value;
    #elif defined( LILYGO_T_DECK_PRO )
