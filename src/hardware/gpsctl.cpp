...
static bool gpsctl_probe_ultra_baud( uint32_t baud, bool *saw_rx ) {
    ...
    uint32_t drain_until = millis() + 100;
    while ( (int32_t)( drain_until - millis() ) > 0 ) {
        bool drained = false;
        while ( gps_serial && gps_serial->available() > 0 ) {
...
void gpsctl_setup( void ) {
    ...
    gpsctl_config.load();
        ...
        bool gpsctl_config_changed = false;
        if ( !gpsctl_config.autoon ) {
            gpsctl_config.autoon = true;
            gpsctl_config_changed = true;
        }
        if ( gpsctl_config.RXPin != SHIELD_GPS_RX || gpsctl_config.TXPin != SHIELD_GPS_TX ) {
            ...
            gpsctl_config_changed = true;
            GPSCTL_INFO_LOG( "forcing T-Watch Ultra GPS pins RX %d/TX %d", gpsctl_config.RXPin, gpsctl_config.TXPin );
        }
        if ( gpsctl_config_changed ) {
            gpsctl_config.save();
        }
    #endif
