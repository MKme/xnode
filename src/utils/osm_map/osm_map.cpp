...
static constexpr double OSM_TILE_SOURCE_PX = 256.0;
static constexpr double OSM_MAX_MERCATOR_LAT = 85.05112878;
...
double osm_helper_long2pixelx( double lon, uint32_t z );
double osm_helper_lat2pixely( double lat, uint32_t z );
bool osm_map_project_lon_lat_locked( osm_location_t *osm_location, double lon, double lat, uint16_t *x, uint16_t *y );
...
double osm_helper_long2pixelx( double lon, uint32_t z ) {
    return( ( ( lon + 180.0 ) / 360.0 ) * (double)( 1UL << z ) * OSM_TILE_SOURCE_PX );
}

double osm_helper_lat2pixely( double lat, uint32_t z ) {
    const double clamped_lat = fmax( -OSM_MAX_MERCATOR_LAT, fmin( OSM_MAX_MERCATOR_LAT, lat ) );
    const double latrad = clamped_lat * M_PI / 180.0;
    return( ( 1.0 - asinh( tan( latrad ) ) / M_PI ) * 0.5 * (double)( 1UL << z ) * OSM_TILE_SOURCE_PX );
}

bool osm_map_project_lon_lat_locked( osm_location_t *osm_location, double lon, double lat, uint16_t *x, uint16_t *y ) {
    const double global_px_x = osm_helper_long2pixelx( lon, osm_location->zoom );
    const double global_px_y = osm_helper_lat2pixely( lat, osm_location->zoom );
    const double tile_origin_x = (double)osm_location->tilex * OSM_TILE_SOURCE_PX;
    const double tile_origin_y = (double)osm_location->tiley * OSM_TILE_SOURCE_PX;
    ...
    if ( tile_px_x < 0.0 || tile_px_x >= OSM_TILE_SOURCE_PX || tile_px_y < 0.0 || tile_px_y >= OSM_TILE_SOURCE_PX ) {
        return( false );
    }

    if ( x ) {
        const double scaled_x = tile_px_x * ( osm_location->tilex_dest_px_res / OSM_TILE_SOURCE_PX );
        const double clamped_x = fmax( 0.0, fmin( osm_location->tilex_dest_px_res - 1.0, scaled_x ) );
        *x = (uint16_t)lround( clamped_x );
    }

    if ( y ) {
        const double scaled_y = tile_px_y * ( osm_location->tiley_dest_px_res / OSM_TILE_SOURCE_PX );
        const double clamped_y = fmax( 0.0, fmin( osm_location->tiley_dest_px_res - 1.0, scaled_y ) );
        *y = (uint16_t)lround( clamped_y );
    }
...
bool osm_map_update( osm_location_t *osm_location ) {
    ...
    if ( tile_update || osm_location->tile_server_source_update || osm_location->manual_nav_update ) {
        ...
        osm_location->tiley_res = fabs( osm_helper_tiley2lat( osm_location->tiley, osm_location->zoom ) - osm_helper_tiley2lat( osm_location->tiley + 1, osm_location->zoom ) );
        osm_location->tilex_res = fabs( osm_helper_tilex2long( osm_location->tilex, osm_location->zoom ) - osm_helper_tilex2long( osm_location->tilex + 1, osm_location->zoom ) );
     ...
     */
    if ( osm_map_project_lon_lat_locked( osm_location, osm_location->lon, osm_location->lat, &osm_location->tilex_pos, &osm_location->tiley_pos ) ) {
...
bool osm_map_project_lon_lat( osm_location_t *osm_location, double lon, double lat, uint16_t *x, uint16_t *y ) {
    bool in_view = false;
    ...
    osm_map_take( osm_location );
    in_view = osm_map_project_lon_lat_locked( osm_location, lon, lat, x, y );
    osm_map_give( osm_location );
